...
plot_HR_vars = function(all.conds.df, param, average = TRUE, log.HR = FALSE, no_stripes = TRUE) {
  ...
  p = p + scale_alpha_manual(values = c("-1" = 0.4, "0" = 0.1, "1" = 1))
...
#' For survival summaries generated by HR_summary_for_all, if a parameter is not
#' available for a condition, the parameter is added with zero-values for overall
#' statistics.
#' @param data summary output created by HR_summary_for_all
#' @param param the name of the parameter to fill saturate the data with
#'
#' @export
surv_add_param_fillers = function(dinput, param) {
...
#' Creates piechart summaries from survival summaries generated by HR_summary_for_all
#' @param data summary output created by HR_summary_for_all
#' @param param the name of the parameter to summarize
#' @param average if TRUE, the estimates of the paramater across different runs
#' are averaged
#' @param add.param If TRUE, in case when the parameter value is not available,
#' 0's are added.
#' @export


plot_param_piechart = function(dinput, param, average = TRUE, add.param = FALSE) {

  if (add.param) {
    dinput = surv_add_param_fillers(dinput, param)
  }
  ...
  data.tissues = data.tissues %>%
    mutate(HR.sign = estimate > 1,
           significance = ifelse(sig.effect > 0, TRUE, FALSE),
